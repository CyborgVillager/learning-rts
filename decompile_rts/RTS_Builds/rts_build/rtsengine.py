...
class MainMenu:
    def __init__(self):
        ...
        self.versionSurf, self.versionRect = ui.genText('CyborgVillager ' + str(my.VERSIONNUMBER), (0, 0), my.WHITE,
                                                        ui.BIGFONT)
        self.versionRect.bottomright = (my.WINDOWWIDTH - ui.GAP, my.WINDOWHEIGHT + 20)
        # Button Location on Main Menu
        ...
        self.optionsButton = ui.Button('Options', 0, (0, my.WINDOWHEIGHT), 1, 1)
        self.optionsButton.rect.right = my.HALFWINDOWWIDTH + 50
        self.creditsButton = ui.Button('Credits', 0, (0, my.WINDOWHEIGHT), 1, 1)
        self.creditsButton.rect.right = my.HALFWINDOWWIDTH + 140
    ...
    def update(self):
        ...
        if not self.animateOut:
            ...
            if self.quitButton.rect.y > my.HALFWINDOWHEIGHT + 100:
                ...
                self.optionsButton.rect.y = self.quitButton.rect.y
        ...
        for button in [self.playButton, self.quitButton, self.optionsButton,self.creditsButton]:
        ...
        elif self.optionsButton.isClicked:
            self.animateOut = 'options'
...
class EmbarkMenu:
    ...
    def __init__(self):
        ...
        for data in self.sliderData:
            self.sliders.append(
                ui.Slider((int(my.WINDOWWIDTH / 2 - ui.Slider.size[0] / 2), 200 + i * ui.Slider.size[1] + i * ui.GAP),
...
class OptionsMenu:
    def __init__(self):
        ...
        self.thanksSurf, self.thanksRect = ui.genText('cyborg', (0, 0), my.WHITE, ui.BIGFONT)
        ...
        self.miniwindowButton = ui.Button('Window', 0, (0, 0), 1, 1)
        self.miniwindowButton.rect.midtop = (my.HALFWINDOWWIDTH, my.WINDOWHEIGHT)
        # Credit Button Location @ Credits
        #self.ocButton = ui.Button('Fullscreen', 0, (0, 0), 1, 1)
        #self.ocButton.rect.midtop = (my.HALFWINDOWWIDTH, 455)
        self.animateOut = False

        # Options Page Extra info
    ...
    def update(self):
        ...
        self.backButton.simulate(my.input)
        self.miniwindowButton.simulate(my.input)
        #self.ocButton.simulate(my.input)
        if self.backButton.isClicked:
            self.animateOut = 'main'
        if self.miniwindowButton.isClicked:
            my.loadSettings()
            my.screenInfo = pygame.display.Info()
            my.WINDOWEDMODE = True
        ...
        if not self.animateOut:
            ...
            if self.miniwindowButton.rect.y > my.HALFWINDOWHEIGHT + 10:
                self.miniwindowButton.rect.y -= math.fabs(my.HALFWINDOWHEIGHT + 20 - self.miniwindowButton.rect.y) * 0.1
#            if self.ocButton.rect.y > my.HALFWINDOWHEIGHT + 10:
 #               self.ocButton.rect.y -= math.fabs(my.HALFWINDOWHEIGHT + 10 - self.ocButton.rect.y) * 0.1
            ...
            if self.backButton.rect.y < my.WINDOWHEIGHT - 1:
                animateDone = False
                self.backButton.rect.y += (my.WINDOWHEIGHT + 50 - self.backButton.rect.y) * 0.1
                self.miniwindowButton.rect.y += (my.WINDOWHEIGHT + 50 - self.miniwindowButton.rect.y) * 0.1
...
class CreditsMenu:
    def __init__(self):
        ...
        self.ocButton.rect.midtop = (my.HALFWINDOWWIDTH, 355)
        self.animateOut = False

    # Credit Page Extra info
    def update(self):
        my.input.get()
        my.screen.fill(my.DARKGREEN)
