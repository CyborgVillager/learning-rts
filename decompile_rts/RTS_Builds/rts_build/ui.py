...
import pygame, my, math, random, time, building, sound
...
class Hud:
    ...
    def updateHUD(self, dt):
        ...
        for resource in my.RESOURCENAMEORDER:
            currentWidth += resourceText('%s: %s' % (resource, my.resources[resource]),
...
class Slider:
    ...
    def update(self):
        ...
        if self.currentlyClicked or self.globalBarRect.collidepoint(
                my.input.mousePos) or self.pointerRectGlobal.collidepoint(my.input.mousePos):
...
class BottomBar:
    ...
    def __init__(self):
        self.bounds = pygame.Rect((GAP * 2, my.WINDOWHEIGHT - BottomBar.height - GAP * 4),
        ...
        if my.selectedTroops:
            ...
            if my.input.mouseUnpressed == 3 and my.input.hoveredCell is not None:
                ...
                if len(my.selectedTroops) == 1:
                    ...
                    for y in range(my.input.hoveredCell[1] - halfSquareWidth,
                                   my.input.hoveredCell[1] + halfSquareWidth):
                        for x in range(my.input.hoveredCell[0] - halfSquareWidth,
...
class BuildingMenu(pygame.sprite.Sprite):
    ...
    def genSurf(self):
        ...
        self.imgList = []
        self.topleft = (
        self.building.rect.left - GAP - iconSize - plusMinusSize * 2 - GAP * 4, self.building.tooltip.rect.top)
    ...
    def genTooltips(self, textList):
        ...
        for i in range(len(textList)):
            ...
            self.tooltips.append(
                Tooltip(tooltipText, (self.rect.right + GAP, self.rect.top + i * self.iconSize + i * GAP)))
        self.hoveredTooltip = None

    def handleInput(self):
        if self.alpha > 0: self.alpha -= 20
        if (self.displayRect.collidepoint(
                my.input.hoveredPixel) and self.alpha > 50) or self.building.rect.collidepoint(my.input.hoveredPixel):
            self.alpha += 60
            if self.alpha > 230:
                self.alpha = 230
            my.UIhover = True
...
class Highlight:
    ...
    for colour in colours:
        ...
        for i in range(1, len(colours) + 1):
            IMGS[colour].append(
                pygame.image.load('assets/ui/highlights/highlight ' + colour + str(i) + '.png').convert_alpha())
...
class StatusText(pygame.sprite.Sprite):
    ...
    def __init__(self, text, zoomTo=None, allowRepeats=False):
        ...
        if self.zoomTo:
            ...
            self.tooltip.surf.blit(StatusText.eyeImg,
...
class Minimap:
    ...
    def __init__(self):
        self.rect = pygame.Rect((my.WINDOWWIDTH - my.MAPXCELLS - GAP * 6, my.WINDOWHEIGHT - my.MAPYCELLS - GAP * 6),
    ...
    def update(self):
        ...
        my.screen.blit(self.surf, self.rect)
        my.screen.blit(Minimap.borderImg,
                       (self.rect.left - int(my.MAPXCELLS / 20), self.rect.top - int(my.MAPYCELLS / 20)))
        self.handleInput()
