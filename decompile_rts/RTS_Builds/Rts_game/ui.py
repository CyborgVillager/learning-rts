...
class Slider:
	...
	def update(self):
		...
		if self.currentlyClicked or self.globalBarRect.collidepoint(my.input.mousePos) or self.pointerRectGlobal.collidepoint(
				my.input.mousePos):
	...
	def handleInput(self):
		...
		if self.rect.collidepoint(my.input.mousePos): # avoid unnecessary collision detection
			for i in range(len(self.humanRectsGlobal)):
				if self.humanRectsGlobal[i].collidepoint(my.input.mousePos) or self.plusRectsGlobal[i].collidepoint(
						my.input.mousePos)\
...
class BuildingMenu(pygame.sprite.Sprite):
	...
	def handleInput(self):
		...
		if (self.displayRect.collidepoint(my.input.hoveredPixel) and self.alpha > 50) or self.building.rect.collidepoint(
...
class Minimap:
	...
	def update(self):
		...
		my.screen.blit(self.surf, self.rect)
		my.screen.blit(Minimap.borderImg, (self.rect.left - int(my.MAPXCELLS / 20), self.rect.top - int(
			my.MAPYCELLS / 20)))
		self.handleInput()
